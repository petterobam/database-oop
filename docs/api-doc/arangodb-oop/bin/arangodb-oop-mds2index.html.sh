...
echo ">>>>>>>>>>>开始转化..."
echo "#############################################################"
echo "执行：java -jar $jar_path $conf_path $mds_path $tpl_path $out_path > ./arangodb-oop-mds2index.html.out < /dev/null &"
java -jar $jar_path $conf_path $mds_path $tpl_path $out_path > ./arangodb-oop/log/arangodb-oop-mds2index.html.out < /dev/null &
echo "#############################################################"
echo ">>>>>>>>>>>转化结束！！"
echo "------------------------------------------------------------"