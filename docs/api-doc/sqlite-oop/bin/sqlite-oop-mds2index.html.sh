...
echo ">>>>>>>>>>>开始转化..."
echo "#############################################################"
echo "执行：java -jar $jar_path $conf_path $mds_path $tpl_path $out_path > ./sqlite-oop-mds2index.html.out < /dev/null &"
java -jar $jar_path $conf_path $mds_path $tpl_path $out_path > ./sqlite-oop/log/sqlite-oop-mds2index.html.out < /dev/null &
echo "#############################################################"
echo ">>>>>>>>>>>转化结束！！"
echo "------------------------------------------------------------"